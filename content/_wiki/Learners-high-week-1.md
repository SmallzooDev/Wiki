...
- 이부분 때문에 도메인 하나를 통째로 리팩토링했다, 여러번 질의하거나 내부적으로 외부 컴포넌트를 호출하는 경우 무조건적으로 함수를 분리했고 분리한 함수들 (즉 리졸버에 등록되지 않으면서, 리졸버를 처리하기 위해 질의하는 로직들)을 모듈화 했고, 그 덕분에 쉽게 모킹 할 수 있었다.
```typescript
...
- 이 방법이 좋아보이기는 했지만, postgres를 관리하는데 접근권한이 없었고, 방향을 틀어서 최대한 보상트랜잭션을 넣어주는 방식으로 선회했다.

- 정말 멱등하게 보상트랜잭션 핸들러를 작성했는지는 모르겠고 살짝 아쉽기는 하지만, 이부분은 가지고 있는 고민을 이야기하면서 pl님들과 논의 해 볼 예정이다.
...
gql 관련해서 여기저기 찾아보니, `makeExecutableSchema()`함수에 정말 많은 기능이 있더라.

`requireResolversForNonScalar`, `requireResolversForArgs`와 같은 것들이 있는데, 이런 옵션은 기본값이 false로 되어있다. (너무 빡빡함..)

각각 스칼라객체의 타입이 잘 구현되어있는지, 리졸버의 아규먼트가 전부 있는게 맞는건지 등을 검증하는데, true로 수정해도 커뮤니티쪽에서는 에러가 안잡혔다.

그래도 이부분의 설정을 바꾸는건 가혹할 것 같아서 내가 진행하는 테스트코드에만 적용을 했다.(before에서 한번 검증하고 시작)


### TO-BE

- 가장 큰 도메인이기는 하다고 짐작했지만, 정말로 5년 이상 된 레거시코드의 커버리지를 10% 내외로 끌어올렸다.
