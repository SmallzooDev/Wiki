...
- 그리고 해당 프로젝트는 `Hasura`라는 오픈소스를 사용하는데, 짧게 요약하면 DB스키마를 보고 자동으로 gql api를 만들어준다.(물론 n + 1 관련 이슈들도 잘 해결되어 있도록 최적화 되어있다.)

- 물론 전시영역 관련된 데이터지만, 디비 전부를 인터페이스 화 할 수 는 없기에 그 앞단에 게이트웨이성 nodejs서버를 두고 하수라 엔진에 질의하며 응답을 포워드 해주는 구조이다.
