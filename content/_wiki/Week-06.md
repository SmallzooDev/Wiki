...
lastmod: 2024-06-25 20:33:49 +0900
tags: 
categories: 
description: 
showToc: true
...
> 지금 가정의 단계에서 내부단편화가 발생하는 요인은 `스택과 힙 사이에 사용하지 않는 메모리가 존재`하는 것이다.
...
> 참고로 베이스 + 오프셋(가상주소)이 바운드보다 크면 오류가 발생하는데, 이걸 `Segment Fault`라고 한다.
...
```
근데 우리는 아니까 앞에 4비트를 떼어내서 세그먼트 번호를 구할수 있는데, 컴퓨터는 멍청해서 그게 안된다.
...
> 여기는 번역이 너무 이상하다. 예시에 대한 설명이 부족한데 갑자기 보호비트 이야기가 나와서 당황하스러움..
...
> 이 논의의 대부분은 `사용자 수준의 메모리 할당 라이브러리`의 발전 역사를 중심으로 한다.
