...
lastmod: 2025-06-10 21:02:06 +0900
tags: 
categories: 
description: 
showToc: true
...
## 유틸리티 트레이트
- 유틸리티 트레이트는 크게 세 범주
	- 언어 확장 트레이트 -> Drop, Deref, DerefMut, From, Into (사용자 정의 타입을 언어와 더 밀접하게 통합하기 위해)
	- 마커 트레이트 -> Sized, Copy (주로 제네릭 타입 변수의 바운드 부분에 알려 두어야 할 제약 조건을 표현하기 위해)
	- 공용 어휘 트레이트 -> Default, AsRef, AsMut, Borrow, BorrowMut, TryFrom, TryInto, ToOwned(흔히 겪는 문제에 대한 일반적인 해결책을 제공하기 위해, 쓸데없는 변형을 줄여서 인터페이스를 더 쉽게 이해할 수 있도록)
- 표준 prelude의 drop 함수 `fn drop<T>(_x:T){}`
- Sized를 직접 구현할 수 있고 적용이 필요하면 러스트가 알아서 구현해준다. Sized를 쓸 수 있는 곳은 타입 변수의 바운드 부분밖에 없는데. 이런 종류의 트레이트를 마커 트레이트라고 한다.
- 트레이트 객체와 슬라이스의 포인터는 서로 완벽히 대칭을 이룬다. 
	- 우선 두 경우 모두 자신이 가리키는 타입의 정보만 가지고는 값을 다루기 어렵다. 예를들어 [u8]의 경우 그의 길이를 모르면 인덱스를 쓸 수 없고 `Box<dyn Write>`의 경우 참조하고 있는 값에 알맞은 Write 구현을 모르면 메서드를호출 할 수 없다.
	- 또 둘 다 팻포인터가 자신이 가리키는 타입이 놓치고 있는 정보를 채워주기 위해서 길이나 V테이블 포인터를 같이 들고 있다.
	- 비균일 크기의 타입은 제약이 너무 많아서 Sized타입으로 제한하고 써야한다. (암묵적인 기본값)
	- `struct S<T>`는 사실 `struct S<T:Sized>`이고, 명시적으로 이게 싫다면 `struct S<T:?Sized>`라고 써야 한다.
	- 그런 경우 불확실한 크기를 가졌다고 인지하며 아까 예시에서 str dyn Write와 같은 값들은 팻포인터, 포인터가 된다.
