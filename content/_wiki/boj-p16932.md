...
## Solution
- [rust soulution](https://github.com/SmallzooDev/coding_interview_rust/blob/main/src/bin/b_16932.rs)


## Memo
- 너무 피곤해서 오늘은 쉬운걸로 dp하나 그래프 하나 풀어야지 했는데, dp는 쉽게 골랐지만 bfs를 실패했다.
- 구역별 번호를 메기고 구역별 크기를 메모한다.
- 0으로 되어있는 좌표를 순회하며 이어붙일 수 있는 구역을 식별한다.
- 해당 좌표에 이어붙일 수 있는 모든구역을 max_size로 갱신한다.
```rust
// Baekjoon - 16932  
